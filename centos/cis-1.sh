...
echo "	"$'\e[1;33m'Yellow$'\e[0m'" displays the sections of benchmark"
echo "	"$'\e[1;35m'Purple$'\e[0m'" displays the subsections of benchmark"
echo "	"$'\e[0;33m'\[+\]	Orange$'\e[0m'" displays the subsubsections of benchmark"
echo "Those benchmarks that are N/A are because they must be manually checked by the user"
echo
echo $'\e[1;33m'1 Initial Setup$'\e[0m'
echo --------------------------------------------------------------------------------------
printf "%-77s %s\n" "Title" "Status"
echo --------------------------------------------------------------------------------------
echo $'\e[1;35m'1.1 Filesystem Configuration$'\e[0m'
echo $'\e[0;33m'\[+\]	1.1.1 Disable unused filesystems$'\e[0m'
#1.1.1.1
...
echo $'\e[0;33m'\[+\]	1.1.2 Configure /tmp$'\e[0m'
...
echo $'\e[0;33m'\[+\]	1.1.3 /Configure /var$'\e[0m'
...
echo $'\e[0;33m'\[+\]	1.1.4 /var/tmp$'\e[0m'
...
echo $'\e[0;33m'\[+\]	1.1.5 Configure /var/log$'\e[0m'
...
echo $'\e[0;33m'\[+\]	1.1.6 Configure /var/log/audit$'\e[0m'
...
echo $'\e[0;33m'\[+\]	1.1.7 Configure /home$'\e[0m'
...
echo $'\e[0;33m'\[+\]	1.1.8 Configure /dev/shm$'\e[0m'
...
echo $'\e[1;35m'1.2 Configure Software Updates$'\e[0m'
...
echo $'\e[1;35m'1.3 Filesystem Integrity Checking$'\e[0m'
...
echo $'\e[1;35m'1.4 Secure Boot Settings$'\e[0m'
#1.4.1
...
echo $'\e[1;35m'1.5 Additional Process Hardening$'\e[0m'
#1.5.1
...
echo $'\e[1;35m'1.6 Mandatory Access Control$'\e[0m'
echo $'\e[0;33m'\[+\]	1.6.1 Configure SELinux$'\e[0m'
#1.6.1.1
...
echo $'\e[1;35m'1.7 Command Line Warning Banners$'\e[0m'
#1.7.1
result=`cat /etc/motd`
if [ -z "$motd_check" ]
then
...
echo $'\e[1;35m'1.8 GNOME Display Manager$'\e[0m'
#1.8.1
...
{
	echo $'\e[0;33m'\[+\]   3.4.1 Configure firewalld$'\e[0m'
}
...
{
	echo $'\e[0;33m'\[+\]   3.4.2 Configure nftables$'\e[0m'
}
...
{
	echo $'\e[0;33m'\[+\]   3.4.3 Configure iptables$'\e[0m'
}
...
echo
echo -e "\e[1;33m4 Logging and Auditing\e[0m"
echo --------------------------------------------------------------------------------------
printf "%-77s %s\n" "Title" "Status"
echo --------------------------------------------------------------------------------------
echo -e "\e[1;35m4.1 Configure System Accounting (auditd)\e[0m"
...
then
	printf "%-77s %s\n" "4.1.2.2 Ensure audit logs are not automatically deleted" "$fail"
	let FCOUNTER=FCOUNTER+1
...
then
	...
	printf "%-77s %s\n" "4.1.3.13 Ensure file deletion events by users are collected" "$fail"
	let FCOUNTER=FCOUNTER+1
fi
...
{
output=""
if rpm -q cronie >/dev/null;
then
    	[ -e /etc/cron.deny ] && output=$( echo -e "[${lightred}FAIL${normal}]" )
        if [ ! -e /etc/cron.allow ];
        then
            	output=$( echo -e "[${lightred}FAIL${normal}]" )
		let FCOUNTER=FCOUNTER+1
	else
            	! stat -Lc "%a" /etc/cron.allow | grep -Eq "[0,2,4,6]00" && output=$( echo -e "[${lightred}FAIL${normal}]" )
                ! stat -Lc "%u:%g" /etc/cron.allow | grep -Eq "^0:0$" && output=$( echo -e "[${lightred}FAIL${normal}]" )
		let FCOUNTER=FCOUNTER+1
        fi
	if [ ! -e /etc/cron.deny ] && [ -e /etc/cron.allow ] &&  stat -Lc "%a" /etc/cron.allow | grep -Eq "[0,2,4,6]00" \ &&  stat -Lc "%u:%g" /etc/cron.allow | grep -Eq "^0:0$";
        then
            	output=$( echo -e "[${lightgreen}PASS${normal}]" )
		let PCOUNTER=PCOUNTER+1
...
{
if rpm -q at >/dev/null;
then
    	[ -e /etc/at.deny ] && output=$( echo -e "[${lightred}FAIL${normal}]" )
        if [ ! -e /etc/at.allow ];
        then
            	output=$( echo -e "[${lightred}FAIL${normal}]" )
		let FCOUNTER=FCOUNTER+1
            	...
            	! stat -Lc "%a" /etc/at.allow | grep -Eq "[0,2,4,6]00" && output=$( echo -e "[${lightred}FAIL${normal}]" )
                ! stat -Lc "%u:%g" /etc/at.allow | grep -Eq "^0:0$" && output=$( echo -e "[${lightred}FAIL${normal}]" )
		let FCOUNTER=FCOUNTER+1

	fi
	if [ ! -e /etc/at.deny ] && [ -e /etc/at.allow ] && stat -Lc "%a" /etc/at.allow | grep -Eq "[0,2,4,6]00" \ && stat -Lc "%u:%g" /etc/at.allow | grep -Eq "^0:0$";
        then
            	output=$( echo -e "[${lightgreen}PASS${normal}]" )
		let PCOUNTER=PCOUNTER+1
